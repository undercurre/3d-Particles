...
class ParticleSystem {
  ...
  /** 表演粒子，即用于呈现模型的粒子载体对象 */
  public AnimateEffectParticle?: THREE.Points<
        ...
        if (i.loader != null) {
          ...
          this.defaultLoader.load(i.path, (group) => {
            ...
            if (!(i.NeedRemoveDuplicateParticle === false))
              finalVertices = VerticesDuplicateRemove(finalVertices);
  ...
  private _finishLoadModal() {
    ...
    this.modelList.forEach((val) => {
      maxParticlesCount = Math.max(
        maxParticlesCount,
        val.attributes.position.count
      );
    });
    ...
    AnimateEffectGeometry.setAttribute(
      "position",
      new THREE.Float32BufferAttribute(vertices, 3, false)
    );
    this.AnimateEffectParticle = new THREE.Points(
      AnimateEffectGeometry,
      this.PointMaterial
    );
    ...
    // 开始监听鼠标移动事件
    // 钩子
    this.onModelsFinishedLoad?.call(
      this,
